# main.py — v4.5 — ТЕСТОВЫЕ НАЧИСЛЕНИЯ КАЖДУЮ МИНУТУ
import os
import asyncio
import logging
import sys
...
# === НАЧИСЛЕНИЯ (ТЕСТОВЫЕ - КАЖДУЮ МИНУТУ) ===
async def minutely_accrual():
    try:
        from decimal import Decimal
        ...
        async with AsyncSessionLocal() as db:
            ...
            for user in users:
                ...
                # Базовый минутный доход (1/60 от часового)
                if invested > 0:
                    # Если есть инвестиции: 25% годовых / 365 дней / 24 часа / 60 минут
                    minutely = (invested * Decimal('0.25')) / Decimal('365') / Decimal('24') / Decimal('60')
                else:
                    # Если нет инвестиций: базовый бесплатный майнинг (1/60 от 0.0005 TON/час)
                    minutely = Decimal('0.0005') / Decimal('60')  # 0.00000833 TON/минуту
                
                user.free_mining_balance += minutely
                user.total_earned += minutely
                updated_count += 1
                total_accrued += minutely
                
                logger.info(f"💰 Начислено {minutely:.8f} TON пользователю {user.user_id} (инвест: {invested})")
            
            await db.commit()
            logger.info(f"💰 ТЕСТ НАЧИСЛЕНИЙ: выполнены для {updated_count} пользователей, всего {total_accrued:.8f} TON")
            
    except Exception as e:
        logger.error(f"❌ Начисления: {e}")

# === ТЕСТОВЫЙ ПЛАНИРОВЩИК (КАЖДУЮ МИНУТУ) ===
async def test_scheduler():
    """ТЕСТОВЫЙ планировщик - начисления каждую минуту"""
    logger.info("⏰ ТЕСТОВЫЙ ПЛАНИРОВЩИК: запущен (начисления каждую минуту)")
    
    # Сначала выполняем начисления сразу при запуске
    await minutely_accrual()
    ...
    while True:
        try:
            ...
            logger.info("🕐 ТЕСТ: Время для начислений (каждую минуту)!")
            await minutely_accrual()
                
        except Exception as e:
            logger.error(f"❌ Ошибка тестового планировщика: {e}")
...
# === ГЛАВНЫЙ ЦИКЛ ===
async def main():
    logger.info("🚀 CRYPTOHUNTER v4.5 - ТЕСТОВЫЕ НАЧИСЛЕНИЯ КАЖДУЮ МИНУТУ")
    ...
    try:
        ...
        await asyncio.gather(
            start_bot(),
            test_scheduler(),  # ← ТЕСТОВЫЙ планировщик
            serve_api(),
